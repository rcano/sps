...
class ScalaSyntax(val input: ParserInput) extends Parser with Basic with Identifiers with Literals {
  ...
  def StableId: Rule0 = rule { optional(Id ~ '.') ~ "super" ~ optional(ClassQualifier) ~ '.' ~ Id | Path ~ '.' ~ Id | Id }
  ...
  def ArgumentExprs: Rule0 = rule { '(' ~ (optional(Exprs ~ ',') ~ PostfixExpr ~ ':' ~ '_' ~ '*' | optional(Exprs)) ~ ')' |
  ...
  def TmplDef: Rule0 = rule { optional("case") ~ ("class" ~ ClassDef | "object" ~ ObjectDef) | "trait" ~ TraitDef }
